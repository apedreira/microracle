...
submiC3 <- function(time_exp, c_exp, p, y_exp, disc_pts) {
  ...
  #(repeated times are deleted) taking into account if drug remains equal,
  # decreases or increases over the time.
  ...
  # Initiate storage variables (numeric for each time, vector agglutinates
  # all solutions)
  sol = 0
  sol_vec = c()
  ...
  for (i in 1:length(time_mod)) {
    sol = N0 * exp(mu_g0 * exp(-k_g * drug_mod[i]) *
                     (1 -  exp(-k_S * time_mod[i])) / k_S - mu_d0 * exp(k_d *
                    drug_mod[i]) * time_mod[i])
