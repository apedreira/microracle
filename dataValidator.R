dataValidatorSingle <- function(data) {
  ...
  if(u == 0 || is.na(u)) {
    msg= showNotification(
      "Initial cell concentration must be provided. Check the data from the
      first row of column nº2.",
      type= "error")
  }
  
  else if(time_length != c_length){
    msg=showNotification(
      "Please, provide a value of drug concentration for each experimental time. 
      Check the data from the column nº3",
      type= "error"
    )
  }
  ...
  if (1 %in%   emptyCols) {
    ...
    for (i in 1:colNumber){
      ...
      c_length = length(unlist(subsetDf[[(2*i+1)]])
                        [!is.na(unlist(subsetDf[[(2*i+1)]]))])
      if(initial_u == 0 || is.na(initial_u)) {
        if (i==1){
        msg= showNotification(
          paste("Initial cell concentration must be provided. Check the data
                from the column",(i*2),"."),
          type= "error")
          break
        }
        ...
        msg = showNotification(
          paste("Please, provide a value of drug concentration for each
                experimental time. Check the data from the column nº",
                (i*2+1),"."),
          type = "error"
        )
        break
...
getEmptyColumns <- function(data) {
  ...
  if(length(emptyCols) > 0) {
    ...
    # Remove odd index from emptyCols vector (cell concentrations has even 
    #  index) despite be odd, 1 is keep to assure that Time col exist
    # If the value is different from 1, the result of adding 1 to the current 
    #  value must be added to the vector. In this way, the CFU/mL columns and 
    # their respective antimicrobial concentration columns will be eliminated
    # from the dataframe.
