...
submic_ddac<- function(time_exp, c_exp, param, y_exp, disc_pts) {
  ...
  # Create a sorted vector with experimental and modeling times (created in base of Discretization points)
  # Repeated times are removed.
  ...
  # Repeated concentrations are removed.
  ...
  Model <- function (t, y, parms) {
    with(as.list(y), {
        ...
        dXl   <- (-ka*Xl-kd*Xl)
        dX    <- (ka*Xl+(kg/(ki+X/(bb*10^10)))*S*X-kd*X)
  ...
  for(i in 1:length(time_mod)){
    drug=drug_mod[i]
    out <-  ode(y = c(Xl=X0,X=0,S=1,Xd=0), times = time_mod, 
                   func = Model, parms = NULL)
    Xlv=out[,2]
    Xv=out[,3]
    sol <- (Xlv[i]+Xv[i])
